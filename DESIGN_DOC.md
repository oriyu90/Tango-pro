...
学習画面の配置はAndroidと同じ「上寄せ」「均等配置」の2種類を`quizArrangementMode`（設定 > 問題画面の配置、JSON保存、既定値 `evenFill`）で切り替える。「均等配置」（既定、既存の挙動）は問題カードの前後に`Spacer()`を置いて画面中央付近へ内容を寄せる。「上寄せ」は同じ`Spacer()`を外し、`frame(minHeight:alignment: .top)`で内容を上端から詰めて表示する。Androidの範囲・タグ指定機能はmacOSには存在せず、範囲別進捗カードもmacOSには実装しない。
