...
class SoundPlayer {
    ...
    private fun playTone(frequency: Double, durationSeconds: Double, volume: Double) {
        try {
    ...
    private fun playSquareTone(frequency: Double, durationSeconds: Double, volume: Double) {
        try {
            val sampleRate = 16000
            val numSamples = (durationSeconds * sampleRate).toInt()
            val generatedSnd = ShortArray(numSamples)
