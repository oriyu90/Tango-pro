...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private suspend fun preloadBuiltInCsvs() {
        if (prefs.getBoolean("builtInCsvsLoaded_v1", false)) return
        val builtIns = listOf(
            "daigaku_juken.csv" to "大学受験用",
            "nankan_daigaku.csv" to "難関大学受験用"
        )
        val app = getApplication<Application>()
        for ((assetName, groupName) in builtIns) {
            try {
                val inputStream = app.assets.open(assetName)
                val reader = BufferedReader(InputStreamReader(inputStream, Charsets.UTF_8))
                val groupId = wordDao.insertGroup(StudyGroup(name = groupName))
                    ...
                    if (line.isNotBlank()) {
                        ...
                        if (parts.size >= 2) {
                            ...
                            if (eng.isNotEmpty() && jpn.isNotEmpty()) {
                                wordsToInsert.add(Word(groupId = groupId, english = eng, japanese = jpn, tag = tag))
                            }
                        }
                    }
                ...
                if (wordsToInsert.isNotEmpty()) wordDao.insertWords(wordsToInsert)
                reader.close()
                inputStream.close()
            } catch (e: Exception) {
                e.printStackTrace()
        ...
        prefs.edit().putBoolean("builtInCsvsLoaded_v1", true).apply()
    }
    ...
    fun importCsvFromUri(context: Context, uri: Uri, customName: String, onComplete: (Boolean, String) -> Unit) {
        isImporting = true
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val groupId = wordDao.insertGroup(StudyGroup(name = name))
    ...
    fun importCsvFromText(customName: String, rawText: String, onComplete: (Boolean, String) -> Unit) {
        if (rawText.isBlank()) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    if (newGroup != null) {
                        changeSelectedGroup(newGroup)
                        onComplete(true, "${readCount}個の単語をテキストからインポートしました！")
                    } else {
                        onComplete(false, "インポートデータの作成に失敗しました。")
                    }
                }
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    fun combineGroups(combinedName: String, selectedIds: List<Long>, onComplete: (Boolean, String) -> Unit) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val newGroupId = wordDao.insertGroup(StudyGroup(name = finalName))
                val wordsToInsert = mutableListOf<Word>()
    ...
    fun startStudySession(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                // If range / tag constraints are enabled, filter them first
                ...
                // Apply specific performance categorization flags on top of the range
    ...
    fun submitAnswer(ans: String) {
        ...
        // Speak the correct English word upon submission if it wasn't spoken beforehand (e.g. JA -> EN)
        if (isTtsEnabled && !isEnglishString(q.questionText) && isEnglishString(q.correctAnswer)) {
            ttsService.speak(q.correctAnswer, ttsVolume)
        }
    }
    ...
    fun speakCurrentQuestionIfNeeded() {
        ...
        if (isEnglishString(q.questionText)) {
            ttsService.speak(q.questionText, ttsVolume)
        }
    }
    ...
    fun speakCurrentQuestionManual() {
        ...
        if (isEnglishString(q.questionText)) {
            ...
            ttsService.speak(q.word.english, ttsVolume)
        }
    }
